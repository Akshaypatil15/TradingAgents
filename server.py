...
async def run_dashboard(agent: str, ticker: str, date: str,
                        results_dir: str, config: dict) -> dict:
    ...
    try:
        ...
        agent = agent.lower()
        valid = ["market", "news", "social", "fundamentals",
                 "bull", "bear", "research", "trader", "risk"]
        ...
        paths = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: generate_dashboards(agent, ticker, date, results_dir, config=config)
        )
    ...
    async def event_generator():
        ...
        if config_data.vendors:
            graph_config["data_vendors"] = {
                "core_stock_apis": config_data.vendors.get("core_stock_apis", DEFAULT_CONFIG["data_vendors"]["core_stock_apis"]),
                "technical_indicators": config_data.vendors.get("technical_indicators", DEFAULT_CONFIG["data_vendors"]["technical_indicators"]),
                "fundamental_data": config_data.vendors.get("fundamental_data", DEFAULT_CONFIG["data_vendors"]["fundamental_data"]),
        ...
        yield json.dumps({"type": "log", "content": f"CONFIG: Provider={config_data.provider} | Model={config_data.model_deep}"})
        yield json.dumps({"type": "log", "content": f"CONFIG: Debate={graph_config['max_debate_rounds']} | Risk={graph_config['max_risk_discuss_rounds']} | Recur={graph_config['max_recur_limit']}"})
        ...
        try:
            ...
            async for chunk in graph.graph.astream(init_state):
                while log_records:
                    ...
                    yield json.dumps({"type": "log", "content": f"[{rec['level']}] {rec['message']}", "level": rec["level"]})
                if await request.is_disconnected():
                ...
                yield json.dumps({"type": "log", "content": f"GRAPH: Chunk keys={list(chunk.keys())}", "stats": current_stats})
                ...
                if "market_report" in chunk_data and chunk_data["market_report"]:
                    ...
                    yield json.dumps({"type": "report", "title": "Market Analysis", "content": chunk_data["market_report"]})
                    yield json.dumps({"type": "log", "content": "AGENT: Market Analyst finished.", "stats": current_stats})
                ...
                if "news_report" in chunk_data and chunk_data["news_report"]:
                    ...
                    yield json.dumps({"type": "log", "content": "AGENT: News Analyst finished.", "stats": current_stats})
                ...
                if "sentiment_report" in chunk_data and chunk_data["sentiment_report"]:
                    ...
                    yield json.dumps({"type": "report", "title": "Social Sentiment", "content": chunk_data["sentiment_report"]})
                    yield json.dumps({"type": "log", "content": "AGENT: Social Analyst finished.", "stats": current_stats})
                ...
                if "fundamentals_report" in chunk_data and chunk_data["fundamentals_report"]:
                    ...
                    yield json.dumps({"type": "report", "title": "Fundamental Analysis", "content": chunk_data["fundamentals_report"]})
                    yield json.dumps({"type": "log", "content": "AGENT: Fundamentals Analyst finished.", "stats": current_stats})
                ...
                if "investment_debate_state" in chunk_data:
                    ...
                    if debate.get("current_response", "").startswith("Bull"):
                        ...
                        yield json.dumps({"type": "log", "content": "RESEARCH: Bull argument presented.", "stats": current_stats})
                    elif debate.get("current_response", "").startswith("Bear"):
                        yield json.dumps({"type": "status", "agent": "bear", "status": "completed"})
                        yield json.dumps({"type": "log", "content": "RESEARCH: Bear argument presented.", "stats": current_stats})
                    if debate.get("judge_decision"):
                        yield json.dumps({"type": "report", "title": "Research Decision", "content": debate["judge_decision"]})
                        yield json.dumps({"type": "log", "content": "MANAGER: Research decision made.", "stats": current_stats})
                ...
                if "trader_investment_plan" in chunk_data and chunk_data["trader_investment_plan"]:
                    ...
                    yield json.dumps({"type": "report", "title": "Execution Plan", "content": chunk_data["trader_investment_plan"]})
                    yield json.dumps({"type": "log", "content": "EXECUTION: Trader plan generated.", "stats": current_stats})
                ...
                if "risk_debate_state" in chunk_data:
                    ...
                    if risk.get("judge_decision"):
                        ...
                        yield json.dumps({"type": "report", "title": "Final Portfolio Decision", "content": risk["judge_decision"]})
                        yield json.dumps({"type": "log", "content": "MANAGER: Risk final decision.", "stats": current_stats})
            ...
            completed_agents = []
            if final_state.get("news_report"):completed_agents.append("news")
            if final_state.get("market_report"):completed_agents.append("market")
            if final_state.get("sentiment_report"):completed_agents.append("social")
            if final_state.get("fundamentals_report"):completed_agents.append("fundamentals")
            debate = final_state.get("investment_debate_state") or {}
            if debate.get("bull_history"):completed_agents.append("bull")
            if debate.get("bear_history"):completed_agents.append("bear")
            if debate.get("judge_decision"):completed_agents.append("research")
            if final_state.get("trader_investment_plan"):completed_agents.append("trader")
            risk = final_state.get("risk_debate_state") or {}
            if risk.get("judge_decision"):completed_agents.append("risk")
...
async def chat_with_report(req: ChatRequest):
    ...
    try:
        ...
        llm = client.get_llm()
        prompt = [("system", "Answer based ONLY on the context."), ("human", f"CONTEXT:\n{req.context}\n\nQUESTION: {req.question}")]
